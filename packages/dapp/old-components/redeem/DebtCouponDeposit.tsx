...
import { ensureERC20Allowance } from "old-components/lib/contracts-shortcuts";
import { formatEther } from "old-components/lib/format";
import { safeParseEther } from "old-components/lib/utils";
...
const UcrNftGenerator = () => {
  ...
  const depositDollarForDebtCoupons = async (amount: BigNumber) => {
    ...
    await (await creditNftManager.connect(signer).exchangeDollarsForCreditNft(amount)).wait();
    refreshBalances();
  };
