...
import { ensureERC1155Allowance } from "old-components/lib/contracts-shortcuts";
...
const UcrNftRedeem = () => {
  ...
  const redeemUcrNftForUad = async (amount: BigNumber) => {
    ...
    if (
      ...
      await (await creditNftManager.connect(signer).redeemCreditNft(debtId, amount)).wait();
      refreshBalances();
      fetchDebts(walletAddress, managedContracts.creditNft);
    }
  };
