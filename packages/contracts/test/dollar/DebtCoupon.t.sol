...
contract DebtCouponTest is LocalTestHelper {
    ...
    function test_mintCouponsWorks() public {
        ...
        uint256 init_balance = DebtCoupon(debtCouponAddress).balanceOf(receiver, expiryBlockNumber);
        vm.prank(admin);
        vm.expectEmit(true, false, false, true);
        emit MintedCoupons(receiver, expiryBlockNumber, 1);
        DebtCoupon(debtCouponAddress).mintCoupons(receiver, mintAmount, expiryBlockNumber);
        uint256 last_balance = DebtCoupon(debtCouponAddress).balanceOf(receiver, expiryBlockNumber);
        assertEq(last_balance - init_balance, mintAmount);

        uint256[] memory holderTokens = DebtCoupon(debtCouponAddress).holderTokens(receiver);
    ...
    function test_burnCouponRevertsWorks() public {
        ...
        vm.prank(admin);
        DebtCoupon(debtCouponAddress).mintCoupons(couponOwner, 10, expiryBlockNumber);
        uint256 init_balance = DebtCoupon(debtCouponAddress).balanceOf(couponOwner, expiryBlockNumber);
        ...
        DebtCoupon(debtCouponAddress).burnCoupons(couponOwner, burnAmount, expiryBlockNumber);
        uint256 last_balance = DebtCoupon(debtCouponAddress).balanceOf(couponOwner, expiryBlockNumber);
    ...
    function test_updateTotalDebt() public {
        ...
        uint256 outStandingTotalDebt = DebtCoupon(debtCouponAddress).getTotalOutstandingDebt();
