...
contract ERC4626Test is Test {
  ...
  function testSingleDepositWithdraw(uint128 amount) public {
    ...
    uint aliceUnderlyingAmount = amount;
    ...
    uint alicePreDepositBal = underlying.balanceOf(alice);

    vm.prank(alice);
    uint aliceShareAmount = vault.deposit(aliceUnderlyingAmount, alice);
  ...
  function testSingleMintRedeem(uint128 amount) public {
    ...
    vm.prank(alice);
    uint aliceUnderlyingAmount = vault.mint(aliceShareAmount, alice);
  ...
  function testMultipleMintDepositRedeemWithdraw() public {
    ...
    uint mutationUnderlyingAmount = 3000;
    ...
    // 1. Alice mints 2000 shares (costs 2000 tokens)
    vm.prank(alice);
    uint aliceUnderlyingAmount = vault.mint(2000, alice);

    uint aliceShareAmount = vault.previewDeposit(aliceUnderlyingAmount);
    assertEq(vault.afterDepositHookCalledCounter(), 1);
    ...
    // 2. Bob deposits 4000 tokens (mints 4000 shares)
    vm.prank(bob);
    uint bobShareAmount = vault.deposit(4000, bob);
    uint bobUnderlyingAmount = vault.previewWithdraw(bobShareAmount);
    ...
    // Sanity check.
    uint preMutationShareBal = aliceShareAmount + bobShareAmount;
    uint preMutationBal = aliceUnderlyingAmount + bobUnderlyingAmount;
