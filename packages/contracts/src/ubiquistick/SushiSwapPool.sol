...
contract SushiSwapPool {
    ...
    constructor(IUbiquityDollarManager _manager) {
        ...
        require(manager.governanceTokenAddress() != address(0), "Governance token address not set");
        // check if pair already exist
        address pool = factory.getPair(manager.dollarTokenAddress(), manager.governanceTokenAddress());
        if (pool == address(0)) {
            pool = factory.createPair(manager.dollarTokenAddress(), manager.governanceTokenAddress());
        }
