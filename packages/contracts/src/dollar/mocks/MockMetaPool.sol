...
contract MockMetaPool {
    ...
    uint256[2] public dy_values = [100 ether, 100 ether];
    uint256 price_cumulative_last = 10 ether;
    ...
    function get_price_cumulative_last() external view returns (uint256) {
    ...
    function get_twap_balances() external view returns(uint256[2] memory) {
        return balances;
    }
    ...
    function updateMockParams (uint256 _price_cumulative_last, uint256 _last_block_timestamp, uint256[2] calldata _twap_balances, uint256[2] calldata _dy_values) public {
