...
contract Bonding is CollectableDust {
    ...
    function uADPriceReset(uint256 amount) external onlyBondingManager {
        ...
        IERC20(manager.dollarTokenAddress()).safeTransfer(
            manager.treasuryAddress(), IERC20(manager.dollarTokenAddress()).balanceOf(address(this))
        );
    }
    ...
    function crvPriceReset(uint256 amount) external onlyBondingManager {
        ...
        IERC20(manager.curve3PoolTokenAddress()).safeTransfer(
            manager.treasuryAddress(), IERC20(manager.curve3PoolTokenAddress()).balanceOf(address(this))
        );
    }
    ...
    function withdraw(uint256 _sharesAmount, uint256 _id) public {
        ...
        require(
            IERC1155Ubiquity(manager.bondingShareAddress()).balanceOf(msg.sender, _id) >= _sharesAmount,
        ...
        IMasterChefV2(manager.masterChefAddress()).withdraw(msg.sender, _sharesAmount, _id);
        ...
        IERC20(manager.stableSwapMetaPoolAddress()).safeTransfer(
            msg.sender, IUbiquityFormulas(manager.formulasAddress()).redeemBonds(_sharesAmount, _currentShareValue, ONE)
        );
    }
