...
interface IUbiquityPool {
    ...
    function mintAlgorithmicUAD(
        uint256 ubq_amount_d18,
        uint256 UAD_out_min
    ) external;

    function mintFractionalUAD(
        uint256 collateral_amount,
        uint256 ubq_amount,
        uint256 UAD_out_min
    ) external;

    function redeem1t1UAD(
        uint256 UAD_amount,
        uint256 COLLATERAL_out_min
    ) external;

    function redeemFractionalUAD(
        uint256 UAD_amount,
        uint256 UBQ_out_min,
        uint256 COLLATERAL_out_min
    ) external;

    function redeemAlgorithmicUAD(
        uint256 UAD_amount,
        uint256 UBQ_out_min
    ) external;
