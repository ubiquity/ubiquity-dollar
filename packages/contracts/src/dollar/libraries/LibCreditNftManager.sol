...
library LibCreditNftManager {
    ...
    function setExpiredCreditNftConversionRate(uint256 rate) internal {
        emit ExpiredCreditNftConversionRateChanged(rate, creditNftStorage().expiredCreditNftConversionRate);
    ...
    function setCreditNftLength(uint256 _creditNftLengthBlocks) internal {
        emit CreditNftLengthChanged(_creditNftLengthBlocks, creditNftStorage().creditNftLengthBlocks);
    ...
    function exchangeDollarsForCreditNft(uint256 amount) internal returns (uint256) {
        ...
        uint256 creditNftToMint = LibCreditNftRedemptionCalculator.getCreditNftAmount(amount);

        // we burn user's dollars.
        IERC20Ubiquity(LibAppStorage.appStorage().dollarTokenAddress).burnFrom(msg.sender, amount);
    ...
    function exchangeDollarsForCredit(uint256 amount) internal returns (uint256) {
        ...
        uint256 creditToMint = LibCreditRedemptionCalculator.getCreditAmount(amount, creditNftStorage().blockHeightDebt);

        // we burn user's dollars.
        IERC20Ubiquity(store.dollarTokenAddress).burnFrom(msg.sender, amount);
        // mint Credit
        UbiquityCreditToken creditToken = UbiquityCreditToken(store.creditTokenAddress);
    ...
    function getCreditReturnedForDollars(uint256 amount) internal view returns (uint256) {
        return LibCreditRedemptionCalculator.getCreditAmount(amount, creditNftStorage().blockHeightDebt);
    }
    ...
    function burnExpiredCreditNftForGovernance(uint256 id, uint256 amount) public returns (uint256 governanceAmount) {
        ...
        require(creditNft.balanceOf(msg.sender, id) >= amount, "User not enough Credit NFT");
        ...
        IERC20Ubiquity governanceToken = IERC20Ubiquity(LibAppStorage.appStorage().governanceTokenAddress);
        governanceAmount = amount / creditNftStorage().expiredCreditNftConversionRate;
    ...
    function burnCreditNftForCredit(uint256 id, uint256 amount) public returns (uint256) {
        ...
        UbiquityCreditToken creditToken = UbiquityCreditToken(LibAppStorage.appStorage().creditTokenAddress);
    ...
    function burnCreditTokensForDollars(uint256 amount) public returns (uint256) {
        ...
        UbiquityCreditToken creditToken = UbiquityCreditToken(LibAppStorage.appStorage().creditTokenAddress);
        require(creditToken.balanceOf(msg.sender) >= amount, "User doesn't have enough Credit pool tokens.");
    ...
    function redeemCreditNft(uint256 id, uint256 amount) public returns (uint256) {
        ...
        uint256 maxRedeemableCreditNft = dollar.balanceOf(address(this)) - creditToken.totalSupply();
    ...
    function mintClaimableDollars() public {
        ...
        uint256 totalMintableDollars = IDollarMintCalculator(address(this)).getDollarsToMint();
        uint256 dollarsToMint = totalMintableDollars - (creditNftStorage().dollarsMintedThisCycle);
        ...
        uint256 totalOutstandingDebt = creditNft.getTotalOutstandingDebt() + creditToken.totalSupply();

        if (currentRedeemableBalance > totalOutstandingDebt) {
            uint256 excessDollars = currentRedeemableBalance - (totalOutstandingDebt);

            IDollarMintExcess dollarsDistributor = IDollarMintExcess(address(this));
