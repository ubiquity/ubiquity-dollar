...
library LibCreditRedemptionCalculator {
    ...
    bytes32 constant CREDIT_REDEMPTION_CALCULATOR_STORAGE_SLOT = bytes32(
        uint256(keccak256("ubiquity.contracts.credit.redemption.calculator.storage")) - 1
    ...
    function getCreditAmount(uint256 dollarsToBurn, uint256 blockHeightDebt) internal view returns (uint256) {
        ...
        require(
            cNFT.getTotalOutstandingDebt() < IERC20(store.dollarTokenAddress).totalSupply(),
            "Credit to Dollar: DEBT_TOO_HIGH"
        );
        bytes16 coef = creditRedemptionCalculatorStorage().coef.fromUInt().div((uint256(1 ether)).fromUInt());
