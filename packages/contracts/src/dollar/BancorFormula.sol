...
contract BancorFormula is BancorPower {
    ...
    bytes16 private immutable TWO = (uint256(2)).fromUInt();
    uint32 private constant MAX_WEIGHT = 1e6;
    uint256 private immutable EULER = 2718281828;
    uint256 private immutable NUMERATOR = 128;
    uint256 private immutable growthRate = 15;
    bytes16 private immutable eulerDivisor = (uint256(1e9)).fromUInt(); 
    bytes16 private immutable rateDivisor = (uint256(1e4)).fromUInt(); 
    bytes16 private immutable inflectionPoint = (uint256(1800)).fromUInt();

    /**
     * @dev given a token ID calculates mint price
     *
     * Formula:
     * Price = e / (1 + EulerNumber ^ (- GrowthFunction(tokenID - inflection point)))
     *
     * @param _tokenID  current token iteration
     *
    ...
    function _calculatePurchasePrice(
        ...
        bytes16 euler = EULER.fromUInt().div(eulerDivisor);
        bytes16 gRate = growthRate.fromUInt().div(rateDivisor);
        bytes16 negative = gRate.sub(gRate.mul(TWO));

        bytes16 part1 = negative.mul(_tokenID.fromUInt().sub(inflectionPoint));

        uint256 part2 = euler.toUInt() ** part1.toUInt();
        bytes16 res = NUMERATOR.fromUInt().div(ONE.sub(part2.fromUInt()));
