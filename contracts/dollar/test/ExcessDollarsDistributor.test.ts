...
describe("ExcessDollarsDistributor", () => {
  ...
  it.skip("distributeDollars should work with limited liquidity", async () => {
    ...
    const dyuAD2DAIBefore = await metaPool[
    ...
    console.log(
      "balancesUADBefore",
      ethers.utils.formatEther(balancesUADBefore)
    );
    const balancesCRVBefore = await metaPool.balances(1);
    console.log(
      "balancesCRVBefore",
      ethers.utils.formatEther(balancesCRVBefore)
    );
    ...
    const dyuAD2DAIAfter = await metaPool[
      "get_dy_underlying(int128,int128,uint256)"
    ](0, 1, ethers.utils.parseEther("1"));
    ...
    ).to.be.lt(ethers.utils.parseEther("0.04"));
    expect(zeroAdrBalAfterBurnLiquidity).to.be.gt(
      zeroAdrBalBeforeBurnLiquidity
    );
    const scBalAfterBurnLiquidity = await UGOVPair.balanceOf(
    ...
    console.log(
      "fourtyPercentAsLP",
      ethers.utils.formatEther(fourtyPercentAsLP)
    );
    console.log(
      "fourtyPercentAsLPBefore",
