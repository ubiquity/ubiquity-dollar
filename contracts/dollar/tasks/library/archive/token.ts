...
total supply:${ethers.utils.formatEther(await uAD.totalSupply())} uAD
      ...
      const dollarMintingCalculatorAddress = await manager.dollarMintingCalculatorAddress();
      const dollarMintingCalculator = (await ethers.getContractAt("DollarMintingCalculator", dollarMintingCalculatorAddress)) as DollarMintingCalculator;
      ...
      ${ethers.utils.formatEther(dollarsToMint)} uAD
        ...
        console.log(`twapPrice :${ethers.utils.formatEther(oraclePriceuAD)} can't calculate dollars to mint
        `);
      }

      const uGOVAdr = await manager.governanceTokenAddress();

      const uGOV = (await ethers.getContractAt("UbiquityGovernance", uGOVAdr)) as UbiquityGovernance;
...
total supply:${ethers.utils.formatEther(await uGOV.totalSupply())} UBQ
      ...
      const uAR = (await ethers.getContractAt("UbiquityAutoRedeem", uARAdr)) as UbiquityAutoRedeem;
...
total supply:${ethers.utils.formatEther(await uAR.totalSupply())} uAR
      ...
      const bondingShare = (await ethers.getContractAt("BondingShare", bondingShareAdr)) as BondingShare;
      ...
      const bondingSharesInEth = ethers.utils.formatEther(await bondingShare.totalSupply());
      console.log(`
bondingShare
...
total supply:${bondingSharesInEth} bondingShares
      ...
      const debtCoupon = (await ethers.getContractAt("DebtCoupon", debtCouponAdr)) as DebtCoupon;
      const totOutstandingDebtInEth = ethers.utils.formatEther(await debtCoupon.getTotalOutstandingDebt());
      console.log(`
debtCoupon
...
Total OutstandingDebt: ${totOutstandingDebtInEth} debtCoupon
      ...
      const metaPoolInEth = ethers.utils.formatEther(await metaPool.totalSupply());
      console.log(`
uAD-3CRV Curve LP
...
total supply:${metaPoolInEth} ${await metaPool.symbol()}`);
      ...
      const sushiSwap = (await ethers.getContractAt("SushiSwapPool", sushiSwapAddr)) as SushiSwapPool;
      ...
      const treasuryLPInEth = ethers.utils.formatEther(await ugovUadPair.balanceOf(treasury));

      const sushiLPInEth = ethers.utils.formatEther(await ugovUadPair.totalSupply());
      const price0CumulativeLast = ethers.utils.formatEther(await ugovUadPair.price0CumulativeLast());
      const price1CumulativeLast = ethers.utils.formatEther(await ugovUadPair.price1CumulativeLast());
...
`);
      ...
      const uniPair = new Pair(new TokenAmount(uniUBQ, reserves[1].toString()), new TokenAmount(uniUAD, reserves[0].toString()));
      const UAD_TO_UBQ = new Route([uniPair], uniUAD);

      const trade = new Trade(UAD_TO_UBQ, new TokenAmount(uniUAD, ethers.utils.parseEther("1").toString()), TradeType.EXACT_INPUT);

      console.log("Mid Price UAD --> UBQ:", UAD_TO_UBQ.midPrice.toSignificant(6));
      console.log("Mid Price UBQ --> UAD:", UAD_TO_UBQ.midPrice.invert().toSignificant(6));
      console.log("-".repeat(45));
      console.log("Execution Price UAD --> UBQ:", trade.executionPrice.toSignificant(6));
      console.log("Mid Price after trade UAD --> UBQ:", trade.nextMidPrice.toSignificant(6));
    },
};
