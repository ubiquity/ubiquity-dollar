...
module.exports = {
  description: "Extract the staking state from the bonding contract V1 for the V2 migration",
  positionalParam: {
    path: ["The path to store migration data", `./${DEFAULT_OUTPUT_NAME}`, types.string],
  },
  action:
    (): ActionType<any> =>
    async (taskArgs: CliArgs, { ethers }) => {
