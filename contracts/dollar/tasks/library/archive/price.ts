...
module.exports = {
  ...
  action:
    (): ActionType<any> =>
    async (_taskArgs, { ethers, getNamedAccounts }) => {
      ...
      let USDT = "";
      let curve3CrvToken = "";
      ({ curveFactory, DAI, USDC, USDT, curve3CrvToken } = await getNamedAccounts());
      ...
      const dyDAI2USDT = await metaPool["get_dy_underlying(int128,int128,uint256)"](indices[0], indices[1], ethers.utils.parseEther("1"));

      const dyuAD2USDC = await metaPool["get_dy_underlying(int128,int128,uint256)"](indices2[0], indices2[1], ethers.utils.parseEther("1"));

      const dyuAD2DAI = await metaPool["get_dy_underlying(int128,int128,uint256)"](indices2[0], indices[0], ethers.utils.parseEther("1"));

      const dyuAD2USDT = await metaPool["get_dy_underlying(int128,int128,uint256)"](indices2[0], indices[1], ethers.utils.parseEther("1"));
