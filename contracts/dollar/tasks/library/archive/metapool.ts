  ...
  action:
    ...
    pool1CRVbal:${ethers.utils.formatEther(pool1CRVbal)}
      ...
      1:${ethers.utils.formatEther(rates[1])}
        `);
      const underBalances = await curvePoolFactory.get_underlying_balances(metaPool.address);
    ...
    USDC indices:${indices2[1].toString()}  `);
      ...
      const decimals = await curvePoolFactory.get_underlying_decimals(metaPool.address);
      console.log(`
