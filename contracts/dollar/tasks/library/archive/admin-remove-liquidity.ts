...
module.exports = {
  description: "adminRemoveLiquidity can push uAD price lower or higher by burning LP token for uAD or 3CRV",
  params: {
  ...
  action:
    (): ActionType<any> =>
    async (taskArgs: TaskArgs, { ethers, network, getNamedAccounts }) => {
      ...
      const expectedUAD = await metaPool["calc_withdraw_one_coin(uint256,int128)"](amount, 0);

      const expectedUADStr = ethers.utils.formatEther(expectedUAD);
      const expectedCRV = await metaPool["calc_withdraw_one_coin(uint256,int128)"](amount, 1);
    ...
    1 3CRV => ${ethers.utils.formatEther(oraclePrice3Crv)} uAD
      ...
      let tx = await metaPool.connect(admin)["remove_liquidity_one_coin(uint256,int128,uint256)"](amount, coinIndex, 0);
