...
contract DebtCouponManager is ERC165, IERC1155Receiver {
  ...
  event ExpiredCouponConvertionRateChanged(uint256 newRate, uint256 previousRate);

  event CouponLengthChanged(uint256 newCouponLengthBlocks, uint256 previousCouponLengthBlocks);
  ...
  function setExpiredCouponConvertionRate(uint256 rate) external onlyCouponManager {
    emit ExpiredCouponConvertionRateChanged(rate, expiredCouponConvertionRate);
    expiredCouponConvertionRate = rate;
  }

  function setCouponLength(uint256 _couponLengthBlocks) external onlyCouponManager {
  ...
  function exchangeDollarsForDebtCoupons(uint256 amount) external returns (uint256) {
    ...
    ICouponsForDollarsCalculator couponCalculator = ICouponsForDollarsCalculator(manager.couponCalculatorAddress());
    uint256 couponsToMint = couponCalculator.getCouponAmount(amount);

    // we burn user's dollars.
    UbiquityAlgorithmicDollar(manager.dollarTokenAddress()).burnFrom(msg.sender, amount);
  ...
  function exchangeDollarsForUAR(uint256 amount) external returns (uint256) {
    ...
    IUARForDollarsCalculator uarCalculator = IUARForDollarsCalculator(manager.uarCalculatorAddress());
    uint256 uarToMint = uarCalculator.getUARAmount(amount, blockHeightDebt);

    // we burn user's dollars.
    UbiquityAlgorithmicDollar(manager.dollarTokenAddress()).burnFrom(msg.sender, amount);
    // mint uAR
    UbiquityAutoRedeem autoRedeemToken = UbiquityAutoRedeem(manager.autoRedeemTokenAddress());
  ...
  /// @param amount the amount of dollars to exchange for coupons
  function getCouponsReturnedForDollars(uint256 amount) external view returns (uint256) {
    ICouponsForDollarsCalculator couponCalculator = ICouponsForDollarsCalculator(manager.couponCalculatorAddress());
  ...
  /// @param amount the amount of dollars to exchange for uAR
  function getUARReturnedForDollars(uint256 amount) external view returns (uint256) {
    IUARForDollarsCalculator uarCalculator = IUARForDollarsCalculator(manager.uarCalculatorAddress());
  ...
  function burnExpiredCouponsForUGOV(uint256 id, uint256 amount) public returns (uint256 uGovAmount) {
    // Check whether debt coupon hasn't expired --> Burn debt coupons.
    DebtCoupon debtCoupon = DebtCoupon(manager.debtCouponAddress());

    require(id <= block.number, "Coupon has not expired");
    require(debtCoupon.balanceOf(msg.sender, id) >= amount, "User not enough coupons");
  ...
  function burnCouponsForAutoRedemption(uint256 id, uint256 amount) public returns (uint256) {
    // Check whether debt coupon hasn't expired --> Burn debt coupons.
    DebtCoupon debtCoupon = DebtCoupon(manager.debtCouponAddress());
  ...
  function burnAutoRedeemTokensForDollars(uint256 amount) public returns (uint256) {
    uint256 twapPrice = _getTwapPrice();
    ...
    UbiquityAlgorithmicDollar uAD = UbiquityAlgorithmicDollar(manager.dollarTokenAddress());
  ...
  function redeemCoupons(uint256 id, uint256 amount) public returns (uint256) {
    uint256 twapPrice = _getTwapPrice();
    ...
    require(autoRedeemToken.totalSupply() <= uAD.balanceOf(address(this)), "There aren't enough uAD to redeem currently");
    uint256 maxRedeemableCoupons = uAD.balanceOf(address(this)) - autoRedeemToken.totalSupply();
  ...
  function mintClaimableDollars() public {
    ...
    uint256 totalMintableDollars = IDollarMintingCalculator(manager.dollarMintingCalculatorAddress()).getDollarsToMint();
    ...
    if (currentRedeemableBalance > totalOutstandingDebt) {
      uint256 excessDollars = currentRedeemableBalance - (totalOutstandingDebt);

      IExcessDollarsDistributor dollarsDistributor = IExcessDollarsDistributor(manager.getExcessDollarsDistributor(address(this)));
