...
contract StakingV2 is CollectableDust, Pausable {
  ...
  event Deposit(address indexed _user, uint256 indexed _id, uint256 _lpAmount, uint256 _stakingShareAmount, uint256 _weeks, uint256 _endBlock);
  ...
  event AddLiquidityFromBond(address indexed _user, uint256 indexed _id, uint256 _lpAmount, uint256 _stakingShareAmount);
  ...
  event Migrated(address indexed _user, uint256 indexed _id, uint256 _lpsAmount, uint256 _sharesAmount, uint256 _weeks);

  modifier onlyStakingManager() {
    require(manager.hasRole(manager.STAKING_MANAGER_ROLE(), msg.sender), "not manager");
    _;
  }

  modifier onlyPauser() {
    require(manager.hasRole(manager.PAUSER_ROLE(), msg.sender), "not pauser");
    _;
  }
  ...
  function uADPriceReset(uint256 amount) external onlyStakingManager {
    ...
    uint256 toTransfer = IERC20(manager.dollarTokenAddress()).balanceOf(address(this));
    IERC20(manager.dollarTokenAddress()).transfer(manager.treasuryAddress(), toTransfer);
  ...
  function crvPriceReset(uint256 amount) external onlyStakingManager {
    ...
    // update twap
    ITWAPOracle(manager.twapOracleAddress()).update();
    uint256 toTransfer = IERC20(manager.curve3PoolTokenAddress()).balanceOf(address(this));
    IERC20(manager.curve3PoolTokenAddress()).transfer(manager.treasuryAddress(), toTransfer);
    emit PriceReset(manager.curve3PoolTokenAddress(), coinWithdrawn, toTransfer);
  }

  function setStakingFormulasAddress(address _stakingFormulasAddress) external onlyStakingManager {
    stakingFormulasAddress = _stakingFormulasAddress;
  }
  ...
  function addLiquidity(
    ...
    (uint256[2] memory bs, StakingShareV2.Bond memory bond) = _checkForLiquidity(_id);

    // calculate pending LP rewards
    uint256 sharesToRemove = bs[0];
    _updateLpPerShare();
    uint256 pendingLpReward = lpRewardForShares(sharesToRemove, bond.lpRewardDebt);

    // add an extra step to be able to decrease rewards if locking end is near
    pendingLpReward = StakingFormulas(this.stakingFormulasAddress()).lpRewardsAddLiquidityNormalization(bond, bs, pendingLpReward);
    // add these LP Rewards to the deposited amount of LP token
    bond.lpAmount += pendingLpReward;
    lpRewards -= pendingLpReward;
    IERC20(manager.stableSwapMetaPoolAddress()).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    uint256 _sharesAmount = IUbiquityFormulas(manager.formulasAddress()).durationMultiply(bond.lpAmount, _weeks, stakingDiscountMultiplier);
    ...
    _updateLpPerShare();
    bond.lpRewardDebt = (IMasterChefV2(manager.masterChefAddress()).getStakingShareInfo(_id)[0] * accLpRewardPerShare) / 1e12;

    StakingShareV2(manager.stakingShareAddress()).updateBond(_id, bond.lpAmount, bond.lpRewardDebt, bond.endBlock);
  ...
  function removeLiquidity(uint256 _amount, uint256 _id) external whenNotPaused {
    (uint256[2] memory bs, StakingShareV2.Bond memory bond) = _checkForLiquidity(_id);
    require(bond.lpAmount >= _amount, "Staking: amount too big");
    // we should decrease the UBQ rewards proportionally to the LP removed
    // sharesToRemove = (staking shares * _amount )  / bond.lpAmount ;
    uint256 sharesToRemove = StakingFormulas(this.stakingFormulasAddress()).sharesForLP(bond, bs, _amount);
    ...
    // staking lp balance - StakingShareV2.totalLP
    IERC20 metapool = IERC20(manager.stableSwapMetaPoolAddress());

    // add an extra step to be able to decrease rewards if locking end is near
    pendingLpReward = StakingFormulas(this.stakingFormulasAddress()).lpRewardsRemoveLiquidityNormalization(bond, bs, pendingLpReward);

    uint256 correctedAmount = StakingFormulas(this.stakingFormulasAddress()).correctedAmountToWithdraw(
      StakingShareV2(manager.stakingShareAddress()).totalLP(),
    ...
    // bond.lpRewardDebt = (staking shares * accLpRewardPerShare) /  1e18;
    ...
    emit RemoveLiquidityFromBond(msg.sender, _id, _amount, correctedAmount, pendingLpReward, sharesToRemove);
  }

  // View function to see pending lpRewards on frontend.
  function pendingLpRewards(uint256 _id) external view returns (uint256) {
    StakingShareV2 staking = StakingShareV2(manager.stakingShareAddress());
    StakingShareV2.Bond memory bond = staking.getBond(_id);
    uint256[2] memory bs = IMasterChefV2(manager.masterChefAddress()).getStakingShareInfo(_id);

    uint256 lpBalance = IERC20(manager.stableSwapMetaPoolAddress()).balanceOf(address(this));
    // the excess LP is the current balance minus the total deposited LP
    if (lpBalance >= (staking.totalLP() + totalLpToMigrate)) {
      uint256 currentLpRewards = lpBalance - (staking.totalLP() + totalLpToMigrate);
      ...
      if (currentLpRewards > lpRewards) {
        ...
        curAccLpRewardPerShare = accLpRewardPerShare + ((newLpRewards * 1e12) / IMasterChefV2(manager.masterChefAddress()).totalShares());
      }
      // we multiply the shares amount by the accumulated lpRewards per share
      // and remove the lp Reward Debt
      return (bs[0] * (curAccLpRewardPerShare)) / (1e12) - (bond.lpRewardDebt);
    }
    return 0;
  }
  ...
  function lpRewardForShares(uint256 amount, uint256 lpRewardDebt) public view returns (uint256 pendingLpReward) {
    if (accLpRewardPerShare > 0) {
      pendingLpReward = (amount * accLpRewardPerShare) / 1e12 - (lpRewardDebt);
    }
  }

  function currentShareValue() public view returns (uint256 priceShare) {
    uint256 totalShares = IMasterChefV2(manager.masterChefAddress()).totalShares();
    // priceShare = totalLP / totalShares
    priceShare = IUbiquityFormulas(manager.formulasAddress()).bondPrice(StakingShareV2(manager.stakingShareAddress()).totalLP(), totalShares, ONE);
  }
  ...
  function _migrate(
    ...
    require(1 <= _weeks && _weeks <= 208, "Duration must be between 1 and 208 weeks");
    ...
    // set masterchef for uGOV rewards
    IMasterChefV2(manager.masterChefAddress()).deposit(user, _sharesAmount, _id);
  ...
  function _updateLpPerShare() internal {
    StakingShareV2 bond = StakingShareV2(manager.stakingShareAddress());
    uint256 lpBalance = IERC20(manager.stableSwapMetaPoolAddress()).balanceOf(address(this));
    ...
    if (lpBalance >= (bond.totalLP() + totalLpToMigrate) && totalShares > 0) {
      uint256 currentLpRewards = lpBalance - (bond.totalLP() + totalLpToMigrate);

      // is there new LP rewards to be distributed ?
      if (currentLpRewards > lpRewards) {
        // we calculate the new accumulated LP rewards per share
        accLpRewardPerShare = accLpRewardPerShare + (((currentLpRewards - lpRewards) * 1e12) / totalShares);
  ...
  function _mint(
    ...
    return StakingShareV2(manager.stakingShareAddress()).mint(to, lpAmount, lpRewardDebt, endBlock);
  }

  function _checkForLiquidity(uint256 _id) internal returns (uint256[2] memory bs, StakingShareV2.Bond memory bond) {
    require(IERC1155Ubiquity(manager.stakingShareAddress()).balanceOf(msg.sender, _id) == 1, "Staking: caller is not owner");
    StakingShareV2 staking = StakingShareV2(manager.stakingShareAddress());
    bond = staking.getBond(_id);
    require(block.number > bond.endBlock, "Staking: Redeem not allowed before staking time");

    ITWAPOracle(manager.twapOracleAddress()).update();
    bs = IMasterChefV2(manager.masterChefAddress()).getStakingShareInfo(_id);
  }
}
