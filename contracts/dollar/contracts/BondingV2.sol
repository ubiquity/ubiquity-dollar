...
contract BondingV2 is CollectableDust, Pausable {
  ...
  event Deposit(address indexed _user, uint256 indexed _id, uint256 _lpAmount, uint256 _bondingShareAmount, uint256 _weeks, uint256 _endBlock);
  ...
  event AddLiquidityFromBond(address indexed _user, uint256 indexed _id, uint256 _lpAmount, uint256 _bondingShareAmount);
  ...
  function setBondingFormulasAddress(address _bondingFormulasAddress) external onlyBondingManager {
    bondingFormulasAddress = _bondingFormulasAddress;
  }
  ...
  function addLiquidity(
    ...
    (uint256[2] memory bs, BondingShareV2.Bond memory bond) = _checkForLiquidity(_id);
    ...
    pendingLpReward = BondingFormulas(this.bondingFormulasAddress()).lpRewardsAddLiquidityNormalization(bond, bs, pendingLpReward);
    ...
    uint256 _sharesAmount = IUbiquityFormulas(manager.formulasAddress()).durationMultiply(bond.lpAmount, _weeks, bondingDiscountMultiplier);
    ...
    _updateLpPerShare();
    bond.lpRewardDebt = (IMasterChefV2(manager.masterChefAddress()).getBondingShareInfo(_id)[0] * accLpRewardPerShare) / 1e12;

    BondingShareV2(manager.bondingShareAddress()).updateBond(_id, bond.lpAmount, bond.lpRewardDebt, bond.endBlock);
  ...
  function removeLiquidity(uint256 _amount, uint256 _id) external whenNotPaused {
    ...
    uint256 sharesToRemove = BondingFormulas(this.bondingFormulasAddress()).sharesForLP(bond, bs, _amount);
    ...
    // add an extra step to be able to decrease rewards if locking end is near
    pendingLpReward = BondingFormulas(this.bondingFormulasAddress()).lpRewardsRemoveLiquidityNormalization(bond, bs, pendingLpReward);
  ...
  function pendingLpRewards(uint256 _id) external view returns (uint256) {
    ...
    uint256[2] memory bs = IMasterChefV2(manager.masterChefAddress()).getBondingShareInfo(_id);
    ...
    if (lpBalance >= (bonding.totalLP() + totalLpToMigrate)) {
      uint256 currentLpRewards = lpBalance - (bonding.totalLP() + totalLpToMigrate);
  ...
  function currentShareValue() public view returns (uint256 priceShare) {
    ...
    priceShare = IUbiquityFormulas(manager.formulasAddress()).bondPrice(BondingShareV2(manager.bondingShareAddress()).totalLP(), totalShares, ONE);
  }
  ...
  function _mint(
    ...
    return BondingShareV2(manager.bondingShareAddress()).mint(to, lpAmount, lpRewardDebt, endBlock);
  }

  function _checkForLiquidity(uint256 _id) internal returns (uint256[2] memory bs, BondingShareV2.Bond memory bond) {
    require(IERC1155Ubiquity(manager.bondingShareAddress()).balanceOf(msg.sender, _id) == 1, "Bonding: caller is not owner");
    BondingShareV2 bonding = BondingShareV2(manager.bondingShareAddress());
    bond = bonding.getBond(_id);
    require(block.number > bond.endBlock, "Bonding: Redeem not allowed before bonding time");
