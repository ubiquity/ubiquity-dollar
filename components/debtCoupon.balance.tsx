...
async function _debtCouponBalance(
  ...
  if (manager && provider) {
    ...
    if (debtCoupon) {
      ...
      if (balances) {
        if (!balances.debtCoupon.eq(rawBalance)) setBalances({ ...balances, debtCoupon: rawBalance });
      }
    }
  }
}
...
const DebtCouponBalance = () => {
  ...
  useEffect(() => {
    _debtCouponBalance(account ? account.address : "", manager, provider, balances, setBalances);
  }, [balances]);
