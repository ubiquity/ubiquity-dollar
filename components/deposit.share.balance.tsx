...
import { BondingShareV2, BondingShareV2__factory, MasterChefV2, MasterChefV2__factory } from "../contracts/artifacts/types";
...
async function calculateBondingShareBalance(addr: string, bondingShare: BondingShareV2, masterchef: MasterChefV2) {
  const ids = await bondingShare.holderTokens(addr);

  let bondingSharesBalance = BigNumber.from("0");

  if (ids && ids.length > 0) {
    bondingSharesBalance = await (await masterchef.getBondingShareInfo(ids[0]))[0];
  }
...
async function calculateBondingShareLPBalance(addr: string, bondingShare: BondingShareV2) {
  const ids = await bondingShare.holderTokens(addr);
  let bondingSharesLPBalance = BigNumber.from("0");
  if (ids && ids.length > 0) {
    bondingSharesLPBalance = await (await bondingShare.getBond(ids[0])).lpAmount;
  }
...
async function _bondingShareBalance(
  ...
  if (manager && provider) {
    ...
    const bondingShare = BondingShareV2__factory.connect(bondingShareAdr, provider);
    const masterchefAdr = await manager.masterChefAddress();
    const masterchef = MasterChefV2__factory.connect(masterchefAdr, provider);
    if (bondingShare) {
      const rawShareBalance = await calculateBondingShareBalance(account, bondingShare, masterchef);
      console.log("##rawShare Balance:", rawShareBalance.toString());
      const rawShareLPBalance = await calculateBondingShareLPBalance(account, bondingShare);
      console.log("##rawShareLP Balance:", rawShareLPBalance.toString());
      if (balances) {
        if (!balances.bondingShares.eq(rawShareBalance)) setBalances({ ...balances, bondingShares: rawShareBalance });
        if (!balances.bondingSharesLP.eq(rawShareLPBalance)) setBalances({ ...balances, bondingSharesLP: rawShareLPBalance });
        const totalShareSupply = await masterchef.totalShares();
        if (totalShareSupply.gt(BigNumber.from(0))) {
          const percentage = rawShareBalance.mul(ethers.utils.parseEther("100")).div(totalShareSupply);
...
const DepositShareBalance = () => {
  ...
  useEffect(() => {
    ...
    _bondingShareBalance(account ? account.address : "", manager, provider, balances, setBalances, setPercentage);
  }, [balances]);
      ...
      <div id="deposit-share-balance">
        {balances && balances.bondingSharesLP && bondingSharePercentage ? (
          <>
            ...
            <span>{parseInt(ethers.utils.formatEther(balances.bondingSharesLP))} LP locked in Bonding Shares </span>
