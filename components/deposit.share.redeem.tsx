...
import { BondingShare__factory, Bonding__factory, UbiquityAlgorithmicDollarManager } from "../contracts/artifacts/types";
...
function DepositShareRedeem(): JSX.Element | null {
  ...
  useEffect(() => {
    ...
    _getDebtIds(account ? account.address : "", manager, provider, debtIds, setDebtIds, blockNum);
  });
  ...
  const redeemBondingShare = redeemBondingShareCurry(provider, account, manager, balances);

  const handleRedeem = handleRedeemCurry(setErrMsg, setIsLoading, redeemBondingShare, debtId, setBalances);
  ...
  return (
    <>
      <div id="debt-coupon-redeem">
        ...
        <input type="number" name="bshareAmount" id="bshareAmount" placeholder="Bonding Shares Amount" value={debtAmount} />
...
function handleRedeemCurry(
  ...
  redeemBondingShare: (debtId: string | undefined, amount: BigNumber, setBalances: Dispatch<SetStateAction<Balances | null>>) => Promise<void>,
  ...
  return async () => {
    setErrMsg("");
    setIsLoading(true);
    const udebtAmount = document.getElementById("bshareAmount") as HTMLInputElement;
...
function redeemBondingShareCurry(
  ...
  return async (debtId: string | undefined, amount: BigNumber, setBalances: Dispatch<SetStateAction<Balances | null>>) => {
    // console.log("debtId", debtId);
    if (provider && account && manager && debtId) {
      await providerAccountManagerAndDebtId(manager, provider, account, amount, debtId, setBalances, balances);
...
async function providerAccountManagerAndDebtId(
  ...
  const bondingShare = BondingShare__factory.connect(bondingShareAdr, provider.getSigner());

  const isAllowed = await bondingShare.isApprovedForAll(account.address, bondingAdr);
  // console.log("isAllowed", isAllowed);
  if (!isAllowed) {
    // first approve
    const approveTransaction = await bondingShare.setApprovalForAll(bondingAdr, true);

    const approveWaiting = await approveTransaction.wait();
    console.log(`approveWaiting gas used with 100 gwei / gas:${ethers.utils.formatEther(approveWaiting.gasUsed.mul(ethers.utils.parseUnits("100", "gwei")))}`);
  }
...
async function managerProviderAndBlockNumber(
  ...
  const bondingShare = BondingShare__factory.connect(await manager.bondingShareAddress(), provider.getSigner());
  ...
  if (
    ...
    debtIds.map((cur, i) => ids[i].eq(BigNumber.from((cur as Option).id))).filter((p) => p === false).length > 0
  ) {
    ...
    for (let index = 0; index < ids.length; index++) {
      ...
      const percent = parseInt(balance.toString()) / parseInt(totalBondingShares.toString());
      console.log({
        balance: balance.toString(),
        totalBondingShares: totalBondingShares.toString(),
        percent,
      });
      const humanReadableBondingShares = parseFloat(ethers.utils.formatEther(balance)).toFixed(0);
...
async function _getDebtIds(
  ...
  if (manager && provider && blockNum) {
    await managerProviderAndBlockNumber(manager, provider, account, debtIds, blockNum, setDebtIds);
  }
}
async function getBlockNumber(provider: ethers.providers.Web3Provider | null, setBlockNumber: Dispatch<SetStateAction<number | undefined>>) {
  let blockNumber = 0;
  if (provider) {
